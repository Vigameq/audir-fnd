...
export class AuditManageComponent {
  ...
  canEditAuditInManage(subAudit: any): boolean {
    return !this.isAuditee && !!subAudit;
  }
  ...
  private softDeleteAudit(audit: any): void {
    const payload: any = {
      audit_id: audit?.audit_id,
    ...
    this.audirService.updateAuditPlan(payload).subscribe((response: any) => {
      ...
      this.audirService.showError('Failed to delete audit');
      console.error('Error deleting audit via update fallback:', updateError);
    });
  }
